...
3. When those merges take happen, the person performing the merge will announce the merge in the `#nmdc-runtime` Slack channel. Although not all team members are on that Slack channel, we think enough _are_ that—should someone else report an issue shortly thereafter—news of the recent merge will make its way to them. The announcement can be a result message (`@`-mentioning people is optional).
