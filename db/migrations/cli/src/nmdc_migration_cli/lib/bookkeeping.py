"""
This module contains the definitions of the `Bookkeeper` class and supporting classes.

This module started out as a verbatim copy of `db/migrations/notebooks/bookkeeper.py`, which is
used by the migration notebooks that we used to use for migration orchestration before we
implemented the migration CLI. We immediately removed this module's dependency on `nmdc-schema`,
though, since the migration CLI doesn't identify `nmdc-schema` as a dependency (it imports it
dynamically).
"""
...
class Bookkeeper:
    ...
    def record_migration_event(
        ...
        name_of_migrator_module: str,
    ) -> None:
        r"""
        Records a migration event in the collection.

        :param event: The type of migration event to record.
        :param from_schema_version: The version of the NMDC Schema the database is being migrated _from_.
        :param to_schema_version: The version of the NMDC Schema the database is being migrated _to_.
        :param name_of_migrator_module: The name of the Python module in which the `Migrator` class used to perform the migration is defined.
        """
