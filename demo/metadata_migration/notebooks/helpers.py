...
class Config:
    ...
    def parse_and_validate_notebook_config_file(self, notebook_config_file_path: str) -> Dict[str, str]:
        ...
        origin_mongo_config_file_path = notebook_config["PATH_TO_ORIGIN_MONGO_CONFIG_FILE"]
        transformer_mongo_config_file_path = notebook_config["PATH_TO_TRANSFORMER_MONGO_CONFIG_FILE"]
        ...
        if not Path(origin_dump_folder_path).parent.is_dir():
            raise FileNotFoundError(f"Parent folder of {origin_dump_folder_path} (origin Mongo dump folder path) not found.")
        if not Path(transformer_dump_folder_path).parent.is_dir():
            raise FileNotFoundError(f"Parent folder of {transformer_dump_folder_path} (transformer Mongo dump folder path) not found.")
    ...
    def parse_and_validate_mongo_config_file(self, mongo_config_file_path: str) -> Dict[str, str]:
