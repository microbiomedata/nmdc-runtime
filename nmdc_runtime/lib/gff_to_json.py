...
def generate_counts(gff3file, md5_sum, informed_by):
    ...
    acceptable_terms = ['cog', 'product', 'smart',
                        'cath_funfam', 'ko', 'ec_number', 'pfam',
                        'superfamily']
    ...
    df = pd.DataFrame(list(count_terms.items()), columns=[
                      'gene_function_id', 'count'])
    ...
    df.to_json(f'{informed_by}_functional_summary.json',
