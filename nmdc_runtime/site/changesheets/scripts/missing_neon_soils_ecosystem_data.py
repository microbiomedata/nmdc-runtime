...
from nmdc_runtime.site.normalization.gold import (
    get_gold_biosample_name_suffix, normalize_gold_id, )
...
def read_omics_processing_to_biosample_map() -> dict:
    ...
    with open(
            DATA_PATH.joinpath("OmicsProcessing-to-catted-Biosamples.tsv")
    ) as f:
        lines_read = 0
        lines_skipped = 0
        for line in f:
            ...
            omics_processing_to_biosamples[
                omics_processing_id] = biosample_ids.split(
                "|"
            )
    logging.debug(
...
def gold_biosample_to_nmdc_biosamples_and_omics_processing_records(
        runtime_api_client, omprc_to_bs_map, goldbs) -> Tuple[
    ...
    nmdcbs_response = runtime_api_client.get_biosamples_by_gold_biosample_id(
        goldbs_id
    )
    ...
    omprc_response = runtime_api_client.get_omics_processing_by_name(
        goldbs_name_suffix
    )
    ...
    for omprc in omprc_records:
        ...
        logging.info(
            f"Found {len(nmdcbs_ids)} NMDC biosamples with {omprc_id}..."
        )
        for nmdcbs_id in nmdcbs_ids:
            nmdcbs_req = runtime_api_client.request(
                "GET", f"/biosamples/{nmdcbs_id}"
            )
    ...
    logging.info(
        f"Found {len(nmdc_biosamples)} NMDC biosamples for {goldbs_id}..."
    )
...
def compare_biosamples(goldbs, nmdcbs) -> List[ChangesheetLineItem]:
    ...
    changesheet_line_items.extend(
        _check_gold_biosample_identifiers(goldbs, nmdcbs)
    )
...
def _check_ecosystem_metadata(goldbs, nmdcbs) -> List[ChangesheetLineItem]:
    ...
    ecosystem_key_map = {"ecosystem": "ecosystem",
    ...
    for nmdc_key, gold_key in ecosystem_key_map.items():
        ...
        if not nmdc_key in nmdcbs:
            changesheet_line_items.append(
                ChangesheetLineItem(
                    id=nmdcbs["id"], action="update", attribute=nmdc_key,
...
def _check_gold_biosample_identifiers(goldbs, nmdcbs) -> List[
    ChangesheetLineItem]:
    changesheet_line_items = []
    goldbs_id = normalize_gold_id(goldbs["biosampleGoldId"])
    if not goldbs_id in nmdcbs["gold_biosample_identifiers"]:
        changesheet_line_items.append(
            ChangesheetLineItem(
                id=nmdcbs["id"], action="insert",
                attribute="gold_biosample_identifiers", value=goldbs_id, )
        )
    return changesheet_line_items
...
def compare_projects(gold_project, omprc_record) -> ChangesheetLineItem:
    ...
    if "alternative_identifiers" not in omprc_record:
        return ChangesheetLineItem(
            id=omprc_record["id"], action="insert",
            attribute="alternative_identifiers", value=gold_project_id, )

    if gold_project_id not in omprc_record["alternative_identifiers"]:
        return ChangesheetLineItem(
            id=omprc_record["id"], action="insert item",
            attribute="alternative_identifiers", value=gold_project_id, )


@click.command()
@click.option(
    "--study_id", default=GOLD_NEON_SOIL_STUDY_ID, help="GOLD study ID"
)
@click.option(
...
def generate_changesheet(study_id, apply_changes, use_dev_api):
    ...
    logging.info(
        f"retrieved {len(gold_biosamples)} biosamples from GOLD API..."
    )
    ...
    for goldbs in gold_biosamples:
        nmdc_biosamples, omprc_records = gold_biosample_to_nmdc_biosamples_and_omics_processing_records(
