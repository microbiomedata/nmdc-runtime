...
from nmdc_runtime.site.ops.badges import award_badges_to_biosamples_op
...
def apply_changesheet():
    ...
    #
    #       It's a variable to whose value we assign no significance. We can use it to tell Dagster
    #       that one op depends upon the output of the other (so Dagster runs them in that order),
    #       without implying to maintainers that its actual value is significant to us. Here's how
    #       we could do that:
    #       ```
    #       an_op_we_want_to_run_after_the_one_that_returns_the_underscore(waits_for=_)
    #       ```
