...
def test_delete_workflow_execution_cascade_deletion(api_user_client):
    ...
    primary_output_data_objects = faker.generate_data_objects(
        quantity=3, name="primary_output_data"
    )
    dependent1_output_data_objects = faker.generate_data_objects(
        quantity=2, name="dependent1_output_data"
    )
    dependent2_output_data_objects = faker.generate_data_objects(
        quantity=2, name="dependent2_output_data"
    )
    external_input_data_objects = faker.generate_data_objects(
        quantity=1, name="external_input_data"
    )
    ...
    dependent_workflow_execution_1 = faker.generate_workflow_executions(
        ...
        has_input=[
            ...
            external_input_data_objects[0][
                "id"
            ],  # Uses external input (should be preserved)
        ],
    ...
    job_records = [
        ...
        },
        {
            ...
            "config": {
                "activity_id": "nmdc:wfrbt-11-unrelated.1"
            },  # Different activity_id, should not be deleted
            "claims": [],
        },
    ]
    ...
    try:
        ...
        assert (
            functional_annotation_agg.count_documents(
                {"was_generated_by": primary_workflow_execution["id"]}
            )
            == 2
        )
        ...
        preserved_data_object_ids = [obj["id"] for obj in input_data_objects] + [
        ...
        functional_annotation_agg.delete_many(
            {
                "gene_function_id": {
                    "$in": [
                        record["gene_function_id"]
                        for record in functional_annotation_records
                    ]
                }
            }
        )
    ...
    try:
        with pytest.raises(requests.exceptions.HTTPError) as exc_info:
            api_user_client.request(
                "DELETE", f"/workflows/workflow_executions/{non_existent_id}"
            )
...
def test_delete_workflow_execution_simple_case(api_user_client):
    ...
    try:
        ...
        # Execute the DELETE request
        response = api_user_client.request(
            "DELETE", f"/workflows/workflow_executions/{workflow_execution['id']}"
        )
        ...
        assert response_data["deleted_workflow_execution_ids"] == [
        ...
        data_object_set.delete_many(
            {"id": {"$in": [input_data_object["id"], output_data_object["id"]]}}
        )
...
def test_delete_workflow_execution_unauthorized_user(api_user_client):
    ...
    assert (
        delete_permission is None
    ), "User should not have delete permissions for this test"
