...
class TestSimulateUpdatesAndCheckReferences:
    def test_it_returns_empty_list_when_operation_does_not_break_any_references(
        self, seeded_db
    ):
        assert (
            simulate_updates_and_check_references(
                ...
                update_cmd=UpdateCommand(
                    ...
                    updates=[
                        UpdateStatement(
                            q={"name": "Study A"}, u={"$set": {"name": "Study Alpha"}}
                        ),
                    ],
                ),
            )
    ...
    def test_it_returns_empty_list_when_operation_repairs_the_references_it_breaks(
        self, seeded_db
    ):
        r"""
        ...
        assert (
            simulate_updates_and_check_references(
                ...
                update_cmd=UpdateCommand(
                    ...
                    updates=[
                        UpdateStatement(
                            q={"name": "Study A"}, u={"$set": {"id": new_study_id}}
                        ),
                        UpdateStatement(
                            q={"name": "Study B"}, u={"$set": {"part_of": new_study_id}}
                        ),
                    ],
                ),
            )
    ...
    def test_it_returns_error_messages_when_breaking_incoming_references(
        self, seeded_db
    ):
        ...
        error_messages = simulate_updates_and_check_references(
            ...
            update_cmd=UpdateCommand(
                ...
                updates=[
                    UpdateStatement(
                        q={"name": "Study A"}, u={"$set": {"id": "nmdc:sty-00-000099"}}
                    ),
                ],
            ),
        )
    ...
    def test_it_returns_error_messages_when_breaking_incoming_reference_from_other_collection_only(
        self, seeded_db
    ):
    ...
    def test_it_returns_error_messages_when_breaking_incoming_reference_from_same_collection_only(
        self, seeded_db
    ):
    ...
    def test_it_returns_error_messages_when_adding_broken_outgoing_reference_to_same_collection(
        self, seeded_db
    ):
        ...
        error_messages = simulate_updates_and_check_references(
            ...
            update_cmd=UpdateCommand(
                ...
                updates=[
                    UpdateStatement(
                        q={"name": "Study A"},
                        u={"$set": {"part_of": "nmdc:sty-00-000099"}},
                    ),
                ],
            ),
        )
    ...
    def test_it_returns_error_messages_when_adding_broken_outgoing_reference_to_other_collection(
        self, seeded_db
    ):
        referrer_id = seeded_db["biosample_set"].find_one({"name": "Biosample A"})["id"]
        error_messages = simulate_updates_and_check_references(
            ...
            update_cmd=UpdateCommand(
                ...
                updates=[
                    UpdateStatement(
                        q={"name": "Biosample A"},
                        u={"$set": {"associated_studies": ["nmdc:sty-00-000099"]}},
                    ),
                ],
            ),
        )
    ...
    def test_it_returns_error_messages_when_document_having_new_id_has_broken_reference(
        self, seeded_db
    ):
        r"""
        ...
        assert (
            seeded_db["biosample_set"].count_documents({"id": new_bsm_id}) == 0
        )  # no such document
        assert (
            seeded_db["study_set"].count_documents({"id": nonexistent_study_id}) == 0
        )  # no such document
        error_messages = simulate_updates_and_check_references(
            db=seeded_db,
            update_cmd=UpdateCommand(
                ...
                updates=[
                    UpdateStatement(
                        q={"name": "Biosample A"},
                        u={"$set": {"associated_studies": [nonexistent_study_id]}},
                    ),
                    UpdateStatement(
                        q={"name": "Biosample A"}, u={"$set": {"id": new_bsm_id}}
                    ),
                ],
            ),
        )
