...
def test_get_insdc_biosample_identifiers():
    ...
    insdc_biosample_identifiers = translator._get_insdc_biosample_identifiers("Gb0000001")
    assert insdc_biosample_identifiers == ['biosample:SAMN00000001', 'biosample:SAMN00000002']

    insdc_biosample_identifiers = translator._get_insdc_biosample_identifiers("Gb0000002")
    assert insdc_biosample_identifiers == ['biosample:SAMN00000003']

    insdc_biosample_identifiers = translator._get_insdc_biosample_identifiers("Gb0000003")
...
def test_get_collection_date():
    ...
    collection_date = translator._get_collection_date({
        "dateCollected": "2023-03-02"
    })
...
def test_get_dataset():
    def test_minter(type, how_many = 1):
        return ['fake:' + str(uuid.UUID(bytes=bytes(random.getrandbits(8) for _ in range(16)), version=4)) for _ in range(how_many)]
    ...
    with open(Path(__file__).parent / 'test_gold_translator_data.yaml') as f:
        ...
        for test_data in test_datasets:
            translator = GoldStudyTranslator(**test_data["input"], id_minter=test_minter)
