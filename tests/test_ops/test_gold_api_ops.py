...
from nmdc_runtime.site.ops import get_gold_study_pipeline_inputs, gold_analysis_projects_by_study, gold_biosamples_by_study, gold_projects_by_study, gold_study
...
def test_gold_analysis_projects_by_study(client_config, op_context):
    with requests_mock.mock() as mock:
        mock.get(f'{client_config["base_url"]}/analysis_projects', json=[{ 'apGoldId': 'Ga0499994' }])
...
def test_gold_study(client_config, op_context):
    with requests_mock.mock() as mock:
        mock.get(f'{client_config["base_url"]}/studies', json=[{ 'studyGoldId': 'Gs0149396' }])
